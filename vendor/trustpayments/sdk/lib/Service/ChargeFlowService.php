...
class ChargeFlowService {
	 ...
	 * @throws \TrustPayments\Sdk\VersioningException
	 * @throws \TrustPayments\Sdk\Http\ConnectionException
	 * @return \TrustPayments\Sdk\Model\ChargeFlow
	 */
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\TrustPayments\Sdk\Model\ChargeFlow',
				'/charge-flow/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\TrustPayments\Sdk\Model\ChargeFlow', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \TrustPayments\Sdk\Model\EntityQuery $query The query restricts the charge flows which are returned by the search. (required)
	 * @throws \TrustPayments\Sdk\ApiException
	 * @throws \TrustPayments\Sdk\VersioningException
	 * @throws \TrustPayments\Sdk\Http\ConnectionException
	 * @return \TrustPayments\Sdk\Model\ChargeFlow[]
