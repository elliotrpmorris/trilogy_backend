...
class UserSpaceRoleService {
	 ...
	 * @return \TrustPayments\Sdk\Model\UserSpaceRole
	 */
	...
	public function addRoleWithHttpInfo($user_id, $space_id, $role_id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\TrustPayments\Sdk\Model\UserSpaceRole',
				'/user-space-role/addRole'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\TrustPayments\Sdk\Model\UserSpaceRole', $response->getHeaders()));
