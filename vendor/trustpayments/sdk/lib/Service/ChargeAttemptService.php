...
class ChargeAttemptService {
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\TrustPayments\Sdk\Model\ChargeAttempt',
				'/charge-attempt/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\TrustPayments\Sdk\Model\ChargeAttempt', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \TrustPayments\Sdk\Model\EntityQuery $query The query restricts the charge attempts which are returned by the search. (required)
	 * @throws \TrustPayments\Sdk\ApiException
	 * @throws \TrustPayments\Sdk\VersioningException
	 * @throws \TrustPayments\Sdk\Http\ConnectionException
	 * @return \TrustPayments\Sdk\Model\ChargeAttempt[]
