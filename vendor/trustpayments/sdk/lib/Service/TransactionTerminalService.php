...
class TransactionTerminalService {
	 ...
	 * @param \TrustPayments\Sdk\Model\TerminalReceiptFetchRequest $request  (required)
	 ...
	 * @return \TrustPayments\Sdk\Model\RenderedTerminalReceipt[]
	 */
	...
	public function fetchReceiptsWithHttpInfo($space_id, $request) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\TrustPayments\Sdk\Model\RenderedTerminalReceipt[]',
				'/transaction-terminal/fetch-receipts'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\TrustPayments\Sdk\Model\RenderedTerminalReceipt[]', $response->getHeaders()));
