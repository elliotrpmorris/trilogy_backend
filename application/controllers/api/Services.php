		...
		if ($data['action'] == '' || $data['api_key'] == '') {
			...
			switch ($data['action']) {
				...
				case 'setSelectedMeal':
					...
					break;
				case 'setSelectedMealSixWeekMultiple':
					$response = $this->setSelectedMealSixWeekMultiple($data);
					break;
				...
				case 'getSixWeekTodayMealAll':
					...
					break;
				case 'getSixWeekAllIngredients':
					$response = $this->getSixWeekAllIngredients($data);
					break;
				case 'getSixWeekAllMealByType':
					$response = $this->getSixWeekAllMealByType($data);
					break;
				...
				case 'setLifeStyleGoal':
					...
					break;
				case 'endLifeStyleGoal':
					$response = $this->endLifeStyleGoal($data);
					break;
				...
				case 'setLifeStyleMealByType':
					...
					break;
				case 'setLifeStyleMultipleMealByType':
					$response = $this->setLifeStyleMultipleMealByType($data);
					break;
				case 'getLifeStyleWeekAllIngredients':
					$response = $this->getLifeStyleWeekAllIngredients($data);
					break;
				case 'getLifeStyleMySelectedMeal':
					$response = $this->getLifeStyleMySelectedMeal($data);
					break;
				case 'getLifeStyleMySelectedMealWeekAll':
					$response = $this->getLifeStyleMySelectedMealWeekAll($data);
					break;
				...
				case 'getLifeStyleMealDetailsByID':
					...
					break;
				case 'getLifeStyleTodayMealAll':
					$response = $this->getLifeStyleTodayMealAll($data);
					break;
				...
				case 'deleteAccount':
					...
					break;
				case 'getGymWorkoutRoutineLifeStyle':
					$response = $this->getGymWorkoutRoutineLifeStyle($data);
					break;
				case 'getGymWorkoutLifeStyle':
					$response = $this->getGymWorkoutLifeStyle($data);
					break;
				case 'setGymWorkoutLifeStyleFavorite':
					$response = $this->setGymWorkoutLifeStyleFavorite($data);
					break;
				case 'removeGymWorkoutLifeStyleFavorite':
					$response = $this->removeGymWorkoutLifeStyleFavorite($data);
					break;
				case 'getGymWorkoutLifeStyleFavoriteList':
					$response = $this->getGymWorkoutLifeStyleFavoriteList($data);
					break;
				default:
					$response['success'] = 0;
			...
			if (isset($result) && !empty($result)) {
				foreach ($result as $key => $val) {
					$batchData[] = array(
						...
						"id" =>  $val->id,
						"batch_date" =>  date('dS M Y', $val->batch_date),
					);
				}
			}
 ...
 */
							...
							if (isset($packageData) && !empty($packageData)) {
								foreach ($packageData as $key => $val) {
									if ($val->package_id == 1) {
										...
										$resultLatestPack = $this->sm->customerPackageDateSixWeek($result->id);
	...
	public function getAssignMealSixWeek($data)
	{
		if (
			...
			} elseif (isset($resultLatestPack->batch_date) && $resultLatestPack->batch_date > strtotime(date('d-m-Y') . '+ 3 days')) {
				$response['success'] = 0;
	...
	public function setSelectedMealSixWeekMultiple($data)
	{
		if (
			...
			$dataPost = array(
				...
				'mealCalorie' => $data['mealCalorie'],
				'day' => json_decode($data['day']),
			);
			/* echo "<pre>";
print_r($dataPost);
die(); */
			$result = $this->sm->setSelectedMealSixWeekMultiple($dataPost);
				...
				if ($result) {
					if (isset($result) && !empty($result)) {
						foreach ($result as $key => $val) {
							...
							$returnData[$key]['meal_type_id'] = $val->meal_type_id;
	...
	public function getMealRecipeforSixWeekByUser($data)
	{
		if (
			...
			if ($result) {
				...
				$resultMealType = $this->db->query("SELECT * FROM `meal_type` WHERE `id` = '".$data['meal_type_id']."'");
				$rowMealType = $resultMealType->row();

				$caloriePer = 0;
				if (isset($rowMealType->calorie_per) && $rowMealType->calorie_per > 0) {
					$caloriePer = $rowMealType->calorie_per;
				} else {
					$caloriePer = 0;
				}
				if (isset($caloriePer) && $caloriePer > 0) {
					$mealCalorie = ($totalCalorie * $caloriePer) / 100;
					if (isset($perDrop) && $perDrop > 0) {
						$mealCalorieReq = $mealCalorie - (($mealCalorie * $perDrop) / 100);
					} else {
						$mealCalorieReq = $mealCalorie;
					}
				} else {
					$mealCalorieReq = $totalCalorie;
				}
				...
				if (isset($result['ingredientData']) && !empty($result['ingredientData'])) {
					foreach ($result['ingredientData'] as $key => $val) {
						...
						if (isset($per_of_meal, $val->calorie_per_gm) && $per_of_meal > 0 && $val->calorie_per_gm > 0) {
							$intakeAmt = round((($mealCalorieReq * $per_of_meal) / 100) / $val->calorie_per_gm);
						} else {
							$intakeAmt = 0;
						}
	...
	/* GET SIX WEEK MEAL ALL */

	public function getSixWeekAllMealByType($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
			|| !isset($data['week']) || $data['week'] == ''
			...
			if (isset($resultLatestPack->batch_end_date) && $resultLatestPack->batch_end_date < time()) {
				...
				$result = $this->sm->getSixWeekAllMealByType($data, $batch_id);
				$dataReturn = array();
				$daySelected = array();
				if (isset($result) && !empty($result)) {
					foreach ($result as $key => $val) {
						$daySelected[$val->day] = $val->meal_title;
					}
				}

				for ($i = 0; $i <= 6; $i++) {
					if (isset($daySelected) && array_key_exists($i, $daySelected)) {
						$dataReturn[$i] = array(
							'day' => $i,
							'meal_title' => $daySelected[$i],
						);
					} else {
						$dataReturn[$i] = array(
							'day' => $i,
	...
	/* GET SIX WEEK ALL INGREDIENTS WEEKLY */

	public function getSixWeekAllIngredients($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
			|| !isset($data['week']) || $data['week'] == ''
			...
			if (isset($resultLatestPack->batch_end_date) && $resultLatestPack->batch_end_date < time()) {
				...
				if (isset($resultMealType) && !empty($resultMealType)) {
					foreach ($resultMealType as $key => $val) {
						...
						$assignMeal = $this->sm->getMySelectedMealSixWeekByWeeek($dataPost, $batch_id);
								...
								if ($returnIngedient) {
									if (isset($returnIngedient['ingredientData']) && !empty($returnIngedient['ingredientData'])) {
										foreach ($returnIngedient['ingredientData'] as $ki => $vi) {
											...
											if (isset($vi->max_amt, $intakeAmt) && $intakeAmt < $vi->max_amt && $intakeAmt > $vi->min_amt) {
												$should_take_unit = strval($intakeAmt);
											} elseif (isset($vi->min_amt, $intakeAmt) && $vi->min_amt > $intakeAmt) {
												$should_take_unit = strval($vi->min_amt);
											} else {
												$should_take_unit = strval($vi->max_amt);
											}
											$ingName = strtolower(trim($vi->ingredient_name));

											if (isset($ingredientArray) && array_key_exists($ingName, $ingredientArray)) {
												$ingredientArray[$ingName]['should_take_unit'] = $ingredientArray[$ingName]['should_take_unit'] + $should_take_unit;
											} else {
												$ingredientArray[$ingName]['should_take_unit'] = $should_take_unit;
												$ingredientArray[$ingName]['should_take_measurement'] = $vi->unit_of_measure;
											}
										}
									}
								}
				...
				$dataReturnArray = [];
				$countArr = 0;
				if (isset($ingredientArray) && !empty($ingredientArray)) {
					foreach ($ingredientArray as $key => $val) {
						$dataReturnArray[$countArr]['foodName'] = $key;
						$dataReturnArray[$countArr]['should_take_unit'] = strval($val['should_take_unit']);
						$dataReturnArray[$countArr]['should_take_measurement'] = $val['should_take_measurement'];
						$countArr++;
					}
				}
	...
	public function setLifeStyleGoal($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
			|| !isset($data['weight_type']) || $data['weight_type'] == ''
			|| !isset($data['week']) || $data['week'] == ''
			...
			if (isset($data['weight_type']) && $data['weight_type'] == "maintain_weight") {
				$result = $this->sm->getUserPersonalProfile($data['cust_id']);
				if ($result) {
					$bmr = $this->calculateBmr($result->age, $result->height, $result->weight, $result->gender, $result->activity_lavel);
					$data['weight'] = 0;
					...
					$dataPost['week_calorie_less'] = 0;
					$dataPost['day_less_calorie'] = 0;
					...
					if ($result) {
						$response['success'] = 1;
						$response['message'] = 'You have selected to maintain your weight. Now go and select your meals.';
					} else {
						$response['success'] = 0;
						$response['message'] = 'Sorry!! no data found.';
					}
			...
			} elseif (isset($data['weight_type']) && $data['weight_type'] == "loose_weight") {
	...
	/* END GOAL */
	public function endLifeStyleGoal($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
		) {
			...
			$result = $this->sm->endLifeStyleGoal($data['cust_id']);

			if ($result) {
				$response['success'] = 1;
				$response['data'] = $result;
				$response['message'] = 'Your goal has been successfully ended.';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Sorry!! Please set your goal first.';
			}
		}
		...
		if (
			...
			if (isset($resultLife->week_target) && $resultLife->week_target > 0) {
				...
				for ($i = 0; $i < $week; $i++) {
					...
					for ($day = 0; $day <= 6; $day++) {
						$weekList[$i]['dayList'][$day] = array(
							'date' => date('d-m-Y', strtotime($startDate . ' + ' . $day . ' days')),
							'day_id' => $day,
							'day_name' => 'Day ' . ($day  + 1)
						);
					}
				...
				$response['success'] = 0;
				$response['message'] = 'Please set your Goal first in order to select meals.';
			}
		}
	...
	public function getLifeStyleMealByTypeWeekDay($data)
	{
		if (
			...
			$resultMeal = $this->sm->getLifeStyleMealByTypeWeekDay($data);
			...
			if ($resultMeal) {
				...
				if (isset($resultMeal) && !empty($resultMeal)) {
					foreach ($resultMeal as $key => $val) {

						$resultMealType = $this->sm->getMealTypeById($val->meal_type_id);
						if (isset($resultMealType->calorie_per) && $resultMealType->calorie_per > 0) {
						...
						$returnData[$key]['meal_id'] = $val->meal_id;
						$returnData[$key]['food_type'] = $val->food_type;
						$returnData[$key]['food_type_id'] = $val->food_type_id;
						$returnData[$key]['meal_type_id'] = $val->meal_type_id;
						$returnData[$key]['selectedMeal'] = $val->selectedMeal;
						$returnData[$key]['shouldTakeCalorie'] = round($shouldMealIntakeCalori);
					}
				}
	...
	/* SET MEAL TO MY SELECTED MEAL LIFESTYLE MULTIPLE */
	...
	public function setLifeStyleMultipleMealByType($data)
	{
		if (
			...
			$dataPost = array(
				...
				'mealTypeId' => $data['mealTypeId'],
				'meal_id' => $data['meal_id'],
				'day_id' => json_decode($data['day_id']),
			);

			$result = $this->sm->setLifeStyleMultipleMealByType($dataPost);
	...
	public function getLifeStyleMySelectedMeal($data)
	{
		if (
			...
			$resultMeal = $this->sm->getLifeStyleMySelectedMeal($data);
	...
	/* GET MY SELECTED MEAL BY MEAL TYPE LIFESTYLE FOR ALL WEEK */
	...
	public function getLifeStyleMySelectedMealWeekAll($data)
	{
		if (
			...
			$resultMeal = $this->sm->getLifeStyleMySelectedMealWeekAll($data);
			/* echo "<pre>";
print_r($resultMeal); */
			$returnData = [];
			if ($resultMeal) {
				...
				if (isset($resultMeal) && !empty($resultMeal)) {
					foreach ($resultMeal as $key => $val) {
						$daySelected[$val->day_id] = $val->meal_title;
					}
				}
...
print_r($result); */
			...
			if ($result) {
				$responseData['mealCalorieRequiement'] = round($shouldMealIntakeCalori);
				$responseData['mealData'] = $result['mealData'];
				$responseData['nutriData'] = $result['nutriData'];
					...
					foreach ($result['ingredientData'] as $key => $val) {
						...
						if (isset($val->calorie_per_gm) && $val->calorie_per_gm > 0) {
							if (isset($per_of_meal) && $per_of_meal > 0) {
								$intakeAmt = round(((($shouldMealIntakeCalori * $per_of_meal) / 100) / $val->calorie_per_gm));
							} else {
								$intakeAmt = 0;
							}
						} else {
							$intakeAmt = round(((($shouldMealIntakeCalori * $per_of_meal) / 100)));
						}
	...
	public function getLifeStyleTodayMealAll($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
			|| !isset($data['week']) || $data['week'] == ''
			...
			if (isset($resultMealType) && !empty($resultMealType)) {
				foreach ($resultMealType as $key => $val) {
					...
					$resultMealType = $this->sm->getMealTypeById($val->id);
					if (isset($resultMealType->calorie_per) && $resultMealType->calorie_per > 0) {
						$caroiePer = $resultMealType->calorie_per;
					} else {
						$caroiePer = 0;
					...
					$resultReturn['mealData'][$key]['meal_type'] = $val->meal_type_name;
					$resultReturn['mealData'][$key]['mealCalorie'] = $shouldMealIntakeCalori;
					...
					$resultSelectedMeal = $result = $this->sm->getLifeStyleMySelectedMeal($dataPost);

					if (isset($resultSelectedMeal) && !empty($resultSelectedMeal)) {
						foreach ($resultSelectedMeal as $k => $v) {
							$result = $this->sm->getLifeStyleMealDetailsByID(array('meal_id' => $v->meal_id));



							$resultReturn['mealData'][$key]['meal'][$k]['mealTitle'] = $result['mealData']->meal_title;

							if (isset($result['ingredientData']) && !empty($result['ingredientData'])) {
								foreach ($result['ingredientData'] as $km => $vm) {

									$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km] = new stdClass();
									$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->ingredient_name = $vm->ingredient_name;
									$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->calorie_per_measurement = $vm->calorie_per_gm;
									$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->unit_of_measure = $vm->unit_of_measure;
									$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->per_of_meal = $vm->per_of_meal;
									$per_of_meal = $vm->per_of_meal;

									if (isset($vm->calorie_per_gm) && $vm->calorie_per_gm > 0) {
										if (isset($per_of_meal) && $per_of_meal > 0) {
											$intakeAmt = round(((($shouldMealIntakeCalori * $per_of_meal) / 100) / $vm->calorie_per_gm));
										} else {
											$intakeAmt = 0;
										}
									} else {
									...
									if (isset($vm->max_amt, $intakeAmt) && $intakeAmt < $vm->max_amt && $intakeAmt > $vm->min_amt) {
										$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->should_take_unit = strval($intakeAmt);
									} elseif (isset($vm->min_amt, $intakeAmt) && $vm->min_amt > $intakeAmt) {
										$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->should_take_unit = strval($vm->min_amt);
									} else {
										$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->should_take_unit = strval($vm->max_amt);
									}

									/* $resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->should_take_unit = $intakeAmt; */
									$resultReturn['mealData'][$key]['meal'][$k]['ingredientData'][$km]->should_take_measurement =  $vm->unit_of_measure;
								}
							}
						}
					} else {
						$resultReturn['mealData'][$key]['meal'] = array();
					}
				}
			}
	...
	public function getLifeStyleWeekAllIngredients($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
			|| !isset($data['week']) || $data['week'] == ''
			...
			if (isset($resultMealType) && !empty($resultMealType)) {
				foreach ($resultMealType as $key => $val) {
					...
					$resultSelectedMeal = $result = $this->sm->getLifeStyleMySelectedMealWeek($dataPost);
					if (isset($resultSelectedMeal) && !empty($resultSelectedMeal)) {
						foreach ($resultSelectedMeal as $k => $v) {
							$result = $this->sm->getLifeStyleMealDetailsByID(array('meal_id' => $v->meal_id));
							...
							if (isset($result['ingredientData']) && !empty($result['ingredientData'])) {
								foreach ($result['ingredientData'] as $km => $vm) {
									...
									$ingredient_name = $vm->ingredient_name;
									$per_of_meal = $vm->per_of_meal;
									...
									if (isset($vm->max_amt, $intakeAmt) && $intakeAmt < $vm->max_amt && $intakeAmt > $vm->min_amt) {
										$ingAmt = strval($intakeAmt);
									} elseif (isset($vm->min_amt, $intakeAmt) && $vm->min_amt > $intakeAmt) {
										$ingAmt = strval($vm->min_amt);
									} else {
										$ingAmt = strval($vm->max_amt);
									}

									if (isset($returnMealList) && array_key_exists($ingredient_name, $returnMealList)) {
										$returnMealList[$ingredient_name]['should_take_unit'] = $returnMealList[$ingredient_name]['should_take_unit'] + $ingAmt;
									} else {
										$returnMealList[$ingredient_name]['should_take_unit'] = $ingAmt;
										$returnMealList[$ingredient_name]['should_take_measurement'] =  $vm->unit_of_measure;
									}
								}
							}
						}
			...
			if ($returnMealList) {
				$returnMealAmt = array();
				$count = 0;
				if (isset($returnMealList) && !empty($returnMealList)) {
					foreach ($returnMealList as $key => $val) {
						$returnMealAmt[$count]['ingredient_name'] = $key;
						$returnMealAmt[$count]['should_take_unit'] = $val['should_take_unit'];
						$returnMealAmt[$count]['should_take_measurement'] = $val['should_take_measurement'];
						$count++;
					}
				}
				$response['success'] = 1;
				$response['data'] = $returnMealAmt;
				$response['message'] = 'all meals ingredients data successfully fetched.';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Sorry!! no data found.';
			}
	...
	public function deleteAccount($data)
	{
		if (
			...
			$result = $this->sm->deleteAccount($data['cust_id']);

			if ($result) {
				$response['success'] = 1;
				$response['data'] = $result;
				$response['message'] = 'User account with all its data has been successfuly deleted.';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Sorry!! no data found.';
			}
	...
	/* Life Style Home Workouts routine */
	...
	public function getGymWorkoutRoutineLifeStyle($data)
	{
		if (
			...
			$result = $this->sm->getGymWorkoutRoutineLifeStyle($data);

			if ($result) {
				$response['success'] = 1;
				$response['data'] = $result;
				$response['message'] = 'Gym Workout Routine List successfully fetched.';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Sorry!! no data found.';
			}
	...
	/* Life Style Home Workouts */

	public function getGymWorkoutLifeStyle($data)
	{
		if (
			!isset($data['cust_id']) || $data['cust_id'] == ''
			|| !isset($data['routine_id']) || $data['routine_id'] == ''
		) {
			$response['success'] = 0;
			$response['message'] = 'Invalid data or required parameter missing.';
		} else {

			$result = $this->sm->getGymWorkoutLifeStyle($data);

			if ($result) {
				$response['success'] = 1;
				$response['data'] = $result;
				$response['message'] = 'Gym Workout List successfully fetched.';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Sorry!! no data found.';
			}
	...
	/* SET Life Style Home Workouts To My Favorite */

	public function setGymWorkoutLifeStyleFavorite($data)
	{

		$result = $this->sm->setGymWorkoutLifeStyleFavorite($data);
	...
	/* REMOVE Life Style Home Workouts To My Favorite */

	public function removeGymWorkoutLifeStyleFavorite($data)
	{

		$result = $this->sm->removeGymWorkoutLifeStyleFavorite($data);
	...
	/* GET Life Style Home Workouts My Favorite LIST */

	public function getGymWorkoutLifeStyleFavoriteList($data)
	{

		$result = $this->sm->getGymWorkoutLifeStyleFavoriteList($data['cust_id']);
