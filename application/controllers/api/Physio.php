...
class Physio extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model("api/physio_model", 'wm');
	...
	public function getPhysioProgram()
	{
		...
		if (
			...
			$result = $this->wm->getPhysioProgram($data);
			if ($result) {
				$response['success'] = 1;
	...
	public function selectPhysioProgram()
	{
		$data['cust_id'] = $this->input->post('cust_id');
		...
		if (
			...
			$result = $this->wm->selectPhysioProgram($data);
			if ($result) {
				$response['success'] = 1;
				$response['message'] = 'Physio Program successfully added';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Physio Program already added';
			}
		}
		echo json_encode($response);
	}
	...
	public function removePhysioProgram()
	{
		$data['cust_id'] = $this->input->post('cust_id');
		$data['program_id'] = $this->input->post('program_id');
		/* $data['date'] = $this->input->post('date');
		...
		if (
			...
			$result = $this->wm->removePhysioProgram($data);
			if ($result) {
				$response['success'] = 1;
				$response['message'] = 'Physio Program successfully removed';
			} else {
				$response['success'] = 0;
				$response['message'] = 'Physio Program already removed';
			}
		}
		echo json_encode($response);
	}

	public function mySelectedPhysioProgram()
	{
		$data['cust_id'] = $this->input->post('cust_id');
		/* $data['date'] = $this->input->post('date'); */
		...
		if (
			...
			$result = $this->wm->mySelectedPhysioProgram($data);
			if ($result) {
				$response['success'] = 1;
				$response['data'] = $result;
				$response['message'] = 'Data successfully fetched.';
