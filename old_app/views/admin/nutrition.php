...
<body>
        ...
        <div class="content-wrapper">
          ...
          <div class="page-header">
            ...
            <h4>
              ...
              <button class="btn btn-outline-default btn-rounded btn-icon" data-position="bottom-right"><i class="mdi mdi mdi-silverware-variant"></i></button> Nutrition

            </h4>

            <nav aria-label="breadcrumb">

              <a type="button" href="JavaScript:void(0)" data-id="0" class="btn btn-outline-primary btn-icon-text btn-xl addUpdateNutrition"><i class="mdi mdi mdi-silverware-variant btn-icon-prepend"></i> Add Nutrition</a>
                      ...
                      <tbody>
                        ...
                        ?>
                            <tr>
                              ...
                              <td>
                                <a href="JavaScript:void(0)" data-id="<?php echo $edit_id; ?>" class="btn btn-xs btn-inverse-primary btn-icon addUpdateNutrition">
                                  <i class="mdi mdi-border-color"></i>
                                </a>
                              </td>
                              <td><a href="<?php echo site_url('admincontrl/nutrition/deleteNutrition/' . $val->id); ?>" class="btn btn-sm btn-rounded btn-inverse-danger btn-icon" onclick="return confirm('Are you sure want to delete this record?');">
  ...
  <script>
    $(document).ready(function() {
      $('.addUpdateNutrition').click(function() {
        $('#errorMsg').removeClass('text-danger');
        $('#errorMsg').html('');
        var id = $(this).data('id');
        if (id == 0) {
          $('#nutritionFrm')[0].reset();
          $('#addNutrition').modal('show');
        } else {
          $.ajax({
            url: '<?php echo  base_url(); ?>/admincontrl/nutrition/selectNutritionById',
            data: {
              'id': id,
            },
            type: 'post',
            ...
            success: function(value) {
              ...
              if (data.status == 1) {
                $('#nutritionId').val(data.returnData.id);
                $('#nutrition_name').val(data.returnData.nutrition_name);
      ...
      $('#nutritionFrm').submit(function() {
        $.ajax({
          url: '<?php echo base_url(); ?>/admincontrl/nutrition/addUpdateNutrition',
          data: $('#nutritionFrm').serialize(),
          type: 'post',
          error: function(XMLHttpRequest, textStatus, errorThrown) {
            alert('Oops! Internal Server Error. Please try again!');
          },
...
<div class="modal" id="addNutrition" tabindex="-1">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <h6 class="modal-title" id="exampleModalLongTitle">Add / Update Nutrition</h6>
      ...
      <form action="#" method="post" id="nutritionFrm" name="nutritionFrm" enctype="multipart/form-data">
        <input type="hidden" name="id" id="nutritionId" value="0">
        <div class="modal-body">
          <span id="errorMsg"></span>
          <div class="form-group">

            <label for="exampleInputUsername1">Nutrition Name</label> <i class="text-danger">*</i>

            <input type="text" class="form-control" placeholder="Enter Nutrition Name" id="nutrition_name" name="nutrition_name" autocomplete="off" required>
