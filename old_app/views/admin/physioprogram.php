...
<body>
        ...
        <div class="content-wrapper">
          ...
          <div class="page-header">
            ...
            <h4>
              ...
              <button class="btn btn-outline-default btn-rounded btn-icon" data-position="bottom-right"><i class="mdi mdi mdi-dumbbell"></i></button> Physio Program Name

            </h4>

            <nav aria-label="breadcrumb">

              <a type="button" href="<?php echo site_url('admin/physio/addprogram'); ?>" data-id="0" class="btn btn-outline-primary btn-icon-text btn-xl"><i class="mdi mdi-dumbbell btn-icon-prepend"></i> Add New</a>
                      ...
                      <tbody>
                        ...
                        ?>
                            <tr>
                              ...
                              <td>
                              <a href="<?php echo site_url('admin/physio/updateprogram/' . $val->id); ?>" class="btn btn-xs btn-inverse-primary btn-icon">
                                  <i class="mdi mdi-border-color"></i>
                                </a>
                              </td>
                              <td><a href="<?php echo site_url('admincontrl/physio/deleteProgram/' . $val->id); ?>" class="btn btn-sm btn-rounded btn-inverse-danger btn-icon" onclick="return confirm('Are you sure want to delete this record?');">
  ...
  <script>
    $(document).ready(function() {
      $('.addUpdatePhysioTypes').click(function() {
        $('#errorMsg').removeClass('text-danger');
        $('#errorMsg').html('');
        var id = $(this).data('id');
        if (id == 0) {
          $('#physioTypeFrm')[0].reset();
          $('#addPhysioTypes').modal('show');
          
        } else {
          $.ajax({
            url: '<?php echo  base_url(); ?>/admincontrl/physiomaster/selectPhysioTypesById',
            data: {
              'id': id,
            },
            type: 'post',
      ...
      $('#physioTypeFrm').submit(function() {
        $.ajax({
          url: '<?php echo base_url(); ?>/admincontrl/physiomaster/addUpdatePhysioTypes',
